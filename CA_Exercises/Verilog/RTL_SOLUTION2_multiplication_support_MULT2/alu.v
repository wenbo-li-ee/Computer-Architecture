...
module alu #(
   ...
   parameter integer REG_W  = 320
   )(
      input   wire signed [REG_W-1:0] alu_in_0,
      input   wire signed [REG_W-1:0] alu_in_1,
      input   wire        [       3:0] alu_ctrl,
      output  reg  signed [REG_W-1:0] alu_out,
   ...
   reg signed [REG_W-1:0] sub_out,add_out,and_out,or_out,
                           nor_out,slt_out, sll_out, srl_out, mul_out, vhsum_out;
   ...
   wire [15:0] mul_out_0, mul_out_1, mul_out_2, mul_out_3, mul_out_4;

   assign mul_out_4 = alu_in_0[5*DATA_W-1:4*DATA_W] * alu_in_1[5*DATA_W-1:4*DATA_W];
   assign mul_out_3 = alu_in_0[4*DATA_W-1:3*DATA_W] * alu_in_1[4*DATA_W-1:3*DATA_W];
   assign mul_out_2 = alu_in_0[3*DATA_W-1:2*DATA_W] * alu_in_1[3*DATA_W-1:2*DATA_W];
   assign mul_out_1 = alu_in_0[2*DATA_W-1:DATA_W] * alu_in_1[2*DATA_W-1:DATA_W];
   assign mul_out_0 = alu_in_0[DATA_W-1:0] * alu_in_1[DATA_W-1:0];


   //ARITHMETIC and LOGIC OPERATIONS
   always@(*)begin
      add_out  =   alu_in_0[DATA_W-1:0] + alu_in_1[DATA_W-1:0];
      vhsum_out  =   alu_in_0[5*DATA_W-1:4*DATA_W] + alu_in_0[4*DATA_W-1:3*DATA_W] + alu_in_0[3*DATA_W-1:2*DATA_W] + alu_in_0[2*DATA_W-1:DATA_W] + alu_in_0[DATA_W-1:0];
      sll_out  =   alu_in_0 << alu_in_1;
      srl_out  =   alu_in_0 >> alu_in_1;
      sub_out  =   alu_in_0[DATA_W-1:0] - alu_in_1[DATA_W-1:0];
      ...
      mul_out  = {mul_out_4, mul_out_3, mul_out_2, mul_out_1, mul_out_0};
   end
   ...
   // OVERFLOW DETECTION
   always@(*)begin
      if(alu_in_0[REG_W-1] == alu_in_1[REG_W-1]) begin
         msb_equal_flag = 1'b1;
      end else begin
         msb_equal_flag = 1'b0;
      end
   ...
   always@(*)begin
      if((msb_equal_flag == 1'b1) && (add_out[REG_W-1] != alu_in_0[REG_W-1]))begin
         overflow_add = 1'b1;
      end else begin
         overflow_add = 1'b0;
      end
...
//   assign overflow_sub = (alu_in_0[REG_W-1] == alu_in_1[REG_W-1] && sub_out[REG_W-1] != alu_in_0[REG_W-1]) ? 1 : 0;

   always@(*)begin
      if((msb_equal_flag == 1'b1) && (sub_out[REG_W-1] != alu_in_0[REG_W-1]))begin
         overflow_sub = 1'b1;
      end else begin
         overflow_sub = 1'b0;
      end
